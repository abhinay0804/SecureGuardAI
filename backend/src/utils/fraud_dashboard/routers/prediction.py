...
# Do not fetch collections at import time — fetch inside endpoints so mock DB fallback works
...
"""
    ...
    # Check active provider selection
    ...
    def try_nvidia():
        nvidia_key_path = os.path.join(project_root, ".nvidia_explain_key")
        nvidia_api_key = os.getenv("NVIDIA_API_KEY")
        if not nvidia_api_key and os.path.exists(nvidia_key_path):
            try:
                with open(nvidia_key_path, "r") as f:
                    nvidia_api_key = f.read().strip()
            except Exception as e:
                print(f"Warning: Failed to read .nvidia_explain_key: {e}")

        if nvidia_api_key:
            nvidia_endpoint = os.getenv("NVIDIA_EXPLAINABILITY_URL", "https://integrate.api.nvidia.com/v1/chat/completions")
            ...
            payload = {
                "model": nvidia_model,
                "messages": [
                    {"role": "system", "content": "You are an experienced fraud analyst."},
                    {"role": "user", "content": prompt}
                ],
                "temperature": 0.2,
                "max_tokens": 512
            }
            response = requests.post(nvidia_endpoint, json=payload, headers=headers, timeout=15)
    ...
    def try_gemini():
        gemini_key_path = os.path.join(project_root, ".gemini_explain_key")
        gemini_api_key = os.getenv("GEMINI_API_KEY")
        if not gemini_api_key and os.path.exists(gemini_key_path):
            try:
                with open(gemini_key_path, "r") as f:
                    gemini_api_key = f.read().strip()
            except Exception as e:
                print(f"Warning: Failed to read .gemini_explain_key: {e}")
    ...
    try:
        # 1) If active_provider == "gemini", try Gemini first
        if active_provider == "gemini":
            try:
                ...
                print(f"ERROR: Active Gemini provider failed: {e}")

        # 2) If active_provider == "nvidia" or default, try NVIDIA first
        try:
            res = try_nvidia()
            if res:
                return res
        except Exception as e:
            print(f"ERROR: NVIDIA explainability request failed: {e}")

        # 3) Fallback to Gemini if NVIDIA wasn't active provider or failed
        if active_provider != "gemini":
            try:
                res = try_gemini()
                if res:
                ...
                print(f"ERROR: Fallback Gemini request failed: {e}")

        # 4) Deterministic local fallback explanation
        drivers = []
        if engineered_features.get("high_amount_flag"):
            drivers.append("High transaction amount")
        if engineered_features.get("is_night"):
            drivers.append("Transaction occurred during night hours")
        drivers_text = "; ".join(drivers) if drivers else "No strong drivers identified"

        text = (
            f"Outcome: {('Fraud' if is_fraud else 'Legitimate')} (score {risk_score:.2f}). Key Drivers: {drivers_text}. "
            "Assessment: ML and rule-based signals evaluated; see combined_reasons for details. "
            "Next Actions: Review transaction and customer profile, consider manual review for high risk."
        )
        return text
    except Exception as e:
...
def predict_and_save(transaction: RawTransactionInput):
    ...
    # Insert record (mock collections will accept this as a no-op)
    try:
        insert_res = getattr(predictions_collection, "insert_one", None)
        if callable(insert_res):
            predictions_collection.insert_one(record)
    except Exception as e:
        print(f"Warning: failed to insert prediction record: {e}")
