...
import { ShieldCheck, RefreshCw, Send, Key } from 'lucide-react';
import { apiService } from './src/services/api';
import { useSettings } from './src/contexts/SettingsContext';
import useResponsive from './src/hooks/useResponsive';
import ExplainabilityKeyModal from './src/components/ExplainabilityKeyModal';
...
const FraudDetection = () => {
  ...
  const [showExplainabilityKeyModal, setShowExplainabilityKeyModal] = useState(false);
  ...
  const handleSubmit = async (event) => {
    ...
    const hasKey = await ensureExplainabilityKey();
    if (!hasKey) {
      setLoading(false);
      return;
    }
  ...
  const ensureExplainabilityKey = async () => {
    const hasKey = await apiService.checkExplainabilityKey();
    if (!hasKey) {
      setShowExplainabilityKeyModal(true);
    }
    return hasKey;
  };
  ...
  return (
    <div className="space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={() => setShowExplainabilityKeyModal(true)}
                className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-md bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 transition"
                title="Configure NVIDIA or Gemini API Key"
              >
      ...
      <ExplainabilityKeyModal
        isOpen={showExplainabilityKeyModal}
        onClose={() => setShowExplainabilityKeyModal(false)}
        onSaved={() => setShowExplainabilityKeyModal(false)}
      />
    </div>
  );
};
